  ...
  server.registerTool(
    'bridge_send_message',
    {
      ...
      description:
        ...
        + '**Recipient relay rule**: when YOU receive an inbound bridge message via the `<channel source="agent-bridge">` block, relay a 1-3 sentence summary (sender machine + target + actionable ask) to the user via your harness\'s configured user-facing channel (Telegram, Slack, Discord, native UI, etc.) so the user has live visibility into cross-harness coordination. **Append the running agent-bridge version at the end** of the relay (e.g. `_(agent-bridge v3.14.9)_`) — read it from the BRIDGE-CONTEXT block\'s `agent_bridge_version` field (OpenClaw) or from `claude_code_channel_status` (Claude Code); do NOT hardcode it. The version helps the user spot fleet-wide version drift. Reply via bridge first if needed, THEN relay to the user. Don\'t suppress purely-routine messages — relay them. Exception: pure-noise heartbeats / `bridge_status` polls. See AGENTS.md "Relay inbound bridge messages to the user" + canonical doc docs/relay-to-user.md.\n\n'
